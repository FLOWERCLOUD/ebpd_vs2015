...
void printMesh(  geodesic::Mesh& mesh ,std::string path,std::string filename, int idx);
...
namespace geodesic {
	...
	public:
		...
		// main entry
		void propagate(unsigned source,int depth,void* drawbackback =NULL ,int drawback_index = -1);
	...
	inline void GeodesicAlgorithmExact::propagate(unsigned source,int propagate_depth = 100,void* drawbackback,int drawback_index)
	{
		...
		while (!m_vertex_queue.empty())
		{
			...
			++ count;
//			printMesh(  *m_mesh ,"./data/","propagate_test", count);
			//if( 0 == drawback_index )
			//	( (void (*)(geodesic::Mesh&,int) )drawbackback)(*m_mesh ,count);
			if(count > propagate_depth)
				break;
		}
