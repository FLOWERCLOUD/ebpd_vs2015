...
#include "control/skeleton_ctrl.hpp"
...
static std::string gen_unique_str(const std::vector<std::string>& vec,
                                  ...
                                  int depth)
{
    std::string n_str = str + " (" + Tbx::Std_utils::to_string(depth) + ")";
    if( !Tbx::Std_utils::exists(vec, n_str) )
        return n_str;
    else
        ...
        const std::vector<Loader::Base_anim_eval*>& list)
{
    ...
    for(unsigned i = 0; i < list.size(); i++)
    {
        ...
        if( Tbx::Std_utils::exists(_anim_names, name) )
...
void Toolbar_frames::update_scene()
{
    if(_curr_anim >= 0)
    {
        Cuda_ctrl::_skeleton.set_pose(_anim_list[_curr_anim], _frame); // <- TODO: to be deleted (old architecture)

        if( Cuda_ctrl::_anim_mesh->_incremental_deformation )
