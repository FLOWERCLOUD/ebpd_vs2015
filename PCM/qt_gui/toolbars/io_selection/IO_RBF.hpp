...
public:
    ...
    virtual void mousePressEvent( QMouseEvent* event ){
        ...
        const int x = event->x();
        const int y = event->y();
        _pix_clicked = Vec2((float)x, (float)y);
...
private:
    ...
    {
        TRS tr = _gizmo_tr;
        Vec3 v   = gizmo()->old_frame() * tr._translation;
        Vec3 a   = gizmo()->old_frame() * tr._axis;
        Vec3 org = _old_tr.get_org();
    ...
    void gen_hrbf_samples_from_mesh_selection()
    {
        ...
        std::vector<Vec3> p(set.size());
        std::vector<Vec3> n(set.size());
    ...
    Vec3 get_sample(const Samp_id& id)
    {
        using namespace Cuda_ctrl;
    ...
    Vec3 get_normal(const Samp_id& id)
    {
        using namespace Cuda_ctrl;
        return _gl_widget->rest_pose() ?
    ...
    Vec2 _pix_clicked;  ///< pixel position when mouse click
    Transfo _old_tr;       ///<
};
