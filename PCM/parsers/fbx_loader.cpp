...
void fill_material(FbxNode* node, Abs_mesh& mesh)
{
    for (int i = 0; i < node->GetMaterialCount(); ++i )
    {
        ...
        if( implem == 0 )        implem = GetImplementation(fbx_mat, FBXSDK_IMPLEMENTATION_CGFX);
...
//EObj::Obj_t Fbx_file::fill_with_mesh(Scene_tree& tree, Obj_mesh*& obj_mesh)
//{
//    Obj_mesh* o = new Obj_mesh();
//    obj_mesh = o;
//    Abs_mesh mesh;
//    get_mesh(mesh);
...
//EObj::Obj_t Fbx_file::fill_with_skel(Scene_tree& tree,
//                                     Obj_skeleton*& obj_skel,
//                                     Abs_skeleton& abs_skel)
//{
//    Obj_skeleton* o = new Obj_skeleton();
//    obj_skel = o;
//    get_skeleton(abs_skel);
//    if( abs_skel._bones.size() > 0)
//    {
//        o->load( abs_skel );
//        tree.register_obj( o );
...
//EObj::Flags Fbx_file::fill_scene(Scene_tree& tree, EObj::Flags flag)
//{
//    Abs_skeleton abs_skel;
//
//    Obj_mesh*     mesh = 0;
...
//    EObj::Flags f = 0;
//
//    if( flag == 0 || EObj::test(flag, EObj::MESH) )
//        EObj::set_on(f, fill_with_mesh( tree, mesh) );
//
//    if( flag == 0 || EObj::test(flag, EObj::SKELETON) )
//        EObj::set_on(f, fill_with_skel( tree, skel, abs_skel) );
//
//    // Translate skeleton weights:
//    const int nb_verts = (int)abs_skel._weights.size();
//    std::vector< std::map<int, float> > weights( nb_verts );
//    for (int i = 0; i < nb_verts; ++i) {
//        for (int b = 0; b < (int)abs_skel._weights[i].size(); ++b) {
//            std::pair<int, float> p = abs_skel._weights[i][b];
//            weights[i][p.first] = p.second;
//        }
//    }
//
//    // Load deformer
//    if( EObj::test(f, EObj::SKELETON) && EObj::test(f, EObj::MESH) )
//        mesh->set_deformer( new Obj_mesh::Deformer(mesh, skel, weights) );
//
//    return f;
//}
