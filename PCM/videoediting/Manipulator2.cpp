...
{
	const char Manipulator2::otherAxis[3][2] = { { 1,2 },{ 0,2 },{ 0,1 } };
	const float Manipulator2::cosTable[13] = { 1, 0.87 ,0.5, 0, -0.5, -0.87, -1, -0.87, -0.5, 0, 0.5, 0.87,1 };
	const float Manipulator2::sinTable[13] = { 0, 0.5, 0.87,1, 0.87 ,0.5, 0, -0.5, -0.87, -1, -0.87, -0.5, 0 };
	const float Manipulator2::axisColor[3][3] = { { 1,0,0 },{ 0,1,0 },{ 0,0,1 } };

	Manipulator2::Manipulator2(void)
	{
		isWorking = 0;
	}
	...
	bool Manipulator2::intersectOriginCylinder(const QVector3D& rayOri, const QVector3D& rayDir,
	...
	bool Manipulator2::intersectOriginDisk(const QVector3D& rayOri, const QVector3D& rayDir,
	...
	float Manipulator2::projRayToOriginAxis(const QVector3D& rayOri, const QVector3D& rayDir, const char axis)
	{
		float vOri[] = { rayOri.x(), rayOri.y(), rayOri.z() };
		float vDir[] = { rayDir.x(), rayDir.y(), rayDir.z() };
	...
	float Manipulator2::projRayToLine(const QVector3D& rayOri, const QVector3D& rayDir, const QVector3D& lineOri, const QVector3D& lineDir)
	{
	...
	void Manipulator2::captureObject(QWeakPointer<RenderableObject> object)
	{
		curObject = object;
		if (curObject)
		{
	...
	void Manipulator2::endManipulate()
	{
		RenderableObject* obj = curObject.data();
		if (obj)
		{
	...
	void Manipulator2::releaseObject()
	{
		if (!curObject.isNull())
		{
	...
	void Manipulator2::transformRayToLocal(QVector3D& ori, QVector3D& dir)
	{
