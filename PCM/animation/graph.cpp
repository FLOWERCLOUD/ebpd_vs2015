...
int Graph::push_vertex(const Vec3& v){
...
void Graph::draw(const Tbx::Camera& cam, int current_vertex) const
{
    glAssert( glMatrixMode(GL_MODELVIEW) );
    Vec3 vx   = cam.get_x();
    Vec3 vy   = cam.get_y();
    Vec3 cdir = cam.get_dir();
    Vec3 pcam = cam.get_pos();
    for(unsigned i = 0; i < _vertices.size(); i++)
    {
...
void Graph::set_offset_scale(const Vec3& offset, float scale)
{
