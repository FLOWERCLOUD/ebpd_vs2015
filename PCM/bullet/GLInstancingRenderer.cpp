...
#include "bullet/OpenGLInclude.h"
...
#include "bullet/Shaders/pointSpriteVS.h"
#include "bullet/Shaders/pointSpritePS.h"
#include "bullet/Shaders/instancingVS.h"
#include "bullet/Shaders/instancingPS.h"
#include "bullet/Shaders/createShadowMapInstancingVS.h"
#include "bullet/Shaders/createShadowMapInstancingPS.h"
#include "bullet/Shaders/useShadowMapInstancingVS.h"
#include "bullet/Shaders/useShadowMapInstancingPS.h"
#include "bullet/Shaders/linesPS.h"
#include "bullet/Shaders/linesVS.h"
...
void GLInstancingRenderer::updateGraphObj(int shapeIndex ,int objIndex ,const float* position, const float* quaternion, const float* color, const float* scaling)
{
	b3GraphicsInstance* gfxObj = m_graphicsInstances[shapeIndex];

	int index = objIndex + gfxObj->m_instanceOffset;
	//int index = gfxObj->m_numGraphicsInstances + gfxObj->m_instanceOffset;
...
void GLInstancingRenderer::updateCamera(float projectionMatrix[16] ,float viewMatrix[16])
{
//	m_data->m_activeCamera->setCameraUpAxis(1);
//	m_data->m_activeCamera->setAspectRatio((float)m_screenWidth/(float)m_screenHeight);
//	m_data->m_defaultCamera1.update();

	for (int i=0;i<16;i++)
	{
		m_data->m_projectionMatrix[i] = projectionMatrix[i];
