		...
		case FileIO::OBJ:
			{
				...
				for( auto  vtxbitr = smpset[smp_idx].begin() ; vtxbitr != smpset[smp_idx].end() ;++vtxbitr ){
					Vertex& vtx = **vtxbitr;
					//ColorType pClr = 255*vtx.color();
					outfile<<"v"<<" "<<vtx.x()<<" "<<vtx.y()<<" "<< vtx.z()<<std::endl;
				}
				for( auto  vtxbitr = smpset[smp_idx].begin() ; vtxbitr != smpset[smp_idx].end() ;++vtxbitr ){
					Vertex& vtx = **vtxbitr;
					outfile<<"vn"<<" "<<vtx.nx()<<" "<<vtx.ny()<<" "<<vtx.nz()<<std::endl;

				}
				for(int k = 0 ;k<smpset[smp_idx].num_triangles() ;++k)
				{
					outfile<<"f"<<" "<<smpset[smp_idx].getTriangle(k).get_i_vertex(0)+1<<"//"<<smpset[smp_idx].getTriangle(k).get_i_vertex(0)+1<<" "
						<< smpset[smp_idx].getTriangle(k).get_i_vertex(1)+1<<"//"<<smpset[smp_idx].getTriangle(k).get_i_vertex(1)+1<<" "
						<<smpset[smp_idx].getTriangle(k).get_i_vertex(2)+1<<"//"<<smpset[smp_idx].getTriangle(k).get_i_vertex(2)+1<<std::endl;
				}
				outfile.close();
			}
			break;
