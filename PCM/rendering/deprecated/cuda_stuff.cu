                   ...
                   bool progressive)
{
    ...
    ctx.plane_org       = _potential_plane._org.to_point3() + _potential_plane._normal*_potential_plane._decal;
    ctx.step_len        = _display._ray_marching_step_length;
    ctx.nb_ray_steps     = _display._ray_marching_nb_steps;
    ctx.nb_reflexion    = 0;
    ...
    p_cl.zero_potential = _color.get(Color_ctrl::POTENTIAL_0);
    p_cl.one_potential = _color.get(Color_ctrl::POTENTIAL_1);
                       ...
                       bool progressive)
{
    ...
    if (joint_set.size()) {
        Raytracing::set_bone_to_trace( joint_set, g_skel->skel_id() );
        Raytracing::set_curr_skel_id( g_skel->skel_id() );
    }
