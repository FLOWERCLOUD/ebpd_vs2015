...
{
    if( Loader::Ppm_loader::read(filename, envmapx, envmapy, img_envmap) ){
        img_ok = true;
    }
...
{
    if( Loader::Ppm_loader::read(filename, blobx, bloby, blob_img)){
        blob_ok = true;
    }
}
...
{
    int* img;
    if( Loader::Ppm_loader::read(filename, extrusionx[n], extrusiony[n], img) )
    {
...
static Vec3 generate_vector(float theta, float phi)
{
...
static void compute_spherical_coordinates(const Vec3& d, float& theta, float& phi)
{
    Vec3 u(d.x, 0.f, d.z);
    u = u.normalized();
    float cosphi = u.x;
...
static void compute_light_envmap()
{
    ...
    for(int i = 0; i < size_x-1; i++){
        ...
        for(int j = 0; j < size_y; j++){
            ...
            Vec3 h = generate_vector(theta,phi);
            float r = 0.f, g = 0.f, b = 0.f;
            for(int k = 0; k < 10000; k++){
                Vec3 v = Vec3::random(1.f);
                v = v.normalized();
                float dot = h.dot(v);
                if(dot < 0.f){
                    v = -v;
